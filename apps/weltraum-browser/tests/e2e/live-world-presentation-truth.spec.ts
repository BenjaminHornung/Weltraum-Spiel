...
test("normal and query-gated runtime expose live world presentation truth without renderer ownership", async ({ page }) => {
  ...
  await assertCanvasHasNoFullTruthValues(canvas);
  const obstacleScreenshot = await page.screenshot({ fullPage: false });
  ...
  const lockedScreenshot = await page.screenshot({ fullPage: false });
  await page.locator("#planner-close").click();
  ...
  const evidenceMarkdown = `# Browser Live World Presentation Truth v1 Evidence\n\n` +
  ...
  await Promise.all([
    ...
    writeFile(screenshotPaths.preview, previewScreenshot),
    writeFile(screenshotPaths.locked, lockedScreenshot),
    writeFile(screenshotPaths.obstacles, obstacleScreenshot)
  ]);
});
