...
test("normal Chromium route proves Structural Microvoxel Destruction Core V1", async ({ page }) => {
  ...
  const evidence = await page.evaluate<StructuralEvidence>(async () => {
    ...
    const classification = structural.deriveStructuralComponentClassification(after, { maxVisitedCells: 256, maxComponents: 8 });
    const repeatClassification = structural.deriveStructuralComponentClassification(afterRepeat, { maxVisitedCells: 256, maxComponents: 8 });
    ...
    const anchoredMass = structural.deriveStructuralComponentMassProperties(after, anchored, { maxVisitedCells: 256 });
    const detachedMass = structural.deriveStructuralComponentMassProperties(after, detached, { maxVisitedCells: 256 });
