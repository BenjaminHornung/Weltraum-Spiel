...
async function selectTargetAndAssertPreview(page: Page, target: (typeof largeTargets)[number]): Promise<TargetPreviewEvidence> {
  ...
  const routeStatus = await page.locator("#route-status").innerText();
...
function createMarkdown(results: readonly TargetPreviewEvidence[]): string {
  const rows = results
    .map((result) => `| ${result.targetId} | ${result.label} | ${result.hudDistanceDisplay} | ${result.hudDistanceMeters} | ${result.routeStatus} | ${result.radarStatus} |`)
    .join("\n");
...
test("normal browser runtime exposes and previews the playable large proving-ground targets", async ({ page }) => {
  ...
  for (const target of largeTargets) {
    ...
    await expect(targetOptions).toContainText(target.targetId === "range-500m" ? "~500 m" : target.targetId === "range-1000m" ? "~1.0 km" : "~2.5 km");
  ...
  await selectTargetAndAssertPreview(page, largeTargets[0]);
