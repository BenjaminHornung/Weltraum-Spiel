...
function stablePlanHashFor(result: CourseEvidenceResult): boolean {
...
test("browser autopilot long-range proving ground records representative evidence", async ({ page }) => {
  ...
  expect(courseIds).toEqual(expect.arrayContaining([
    "direct-long",
    "s-curve-obstacles",
    "narrow-corridor",
    "offset-gates",
  ...
  expect(directLongFast?.failureReasonCodes).toHaveLength(0);
  expect(directLongFast?.invalidationReasons).toHaveLength(0);
  expect(directLongFast?.replanRequired).toBe(false);
  ...
  await writeFile(path.join(evidenceDir, "autopilot-long-range-speed-profile-summary.json"), JSON.stringify({ speedComparison, plannerLimits }, null, 2), "utf8");
