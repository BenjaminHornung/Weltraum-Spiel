...
function formatResultLine(result: CourseEvidenceResult): string {
  return `| ${result.courseId} | ${result.profile} | ${result.distanceMeters} | ${result.classification} | ${result.status} | ${result.simulatedSeconds} | ${result.peakSpeed} | ${result.finalSpeed} | ${result.finalDistance} | ${result.minObstacleClearance} | ${result.fuelUsed} | ${result.replanRequired} | ${result.planHashBefore ?? "null"} | ${result.planHashAfter ?? "null"} |`;
}
...
function createMarkdown(summary: Record<string, unknown>, representativeResults: readonly CourseEvidenceResult[], speedComparison: readonly CourseEvidenceResult[]) {
  const speedRows = speedComparison
    .map((result) => `| ${result.courseId} | ${result.profile} | ${result.distanceMeters} | ${result.classification} | ${result.simulatedSeconds} | ${result.peakSpeed} | ${result.finalSpeed} | ${result.fuelUsed} | ${result.planHashBefore ?? "null"} | ${result.planHashAfter ?? "null"} |`)
    .join("\n");
...
Generated by \`apps/weltraum-browser/tests/e2e/autopilot-proving-ground-long-range.spec.ts\` through the query-gated browser TestBridge.
...
| Course | Profile | Distance (m) | Classification | Status | Sim seconds | Peak speed | Final speed | Final distance | Min obstacle clearance | Fuel used | Replan required | Plan hash before | Plan hash after |
| --- | --- | ---: | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | --- | --- | --- |
${representativeRows}

## 2500m Safe / Balanced / Fast Comparison

| Course | Profile | Distance (m) | Classification | Sim seconds | Peak speed | Final speed | Fuel used | Plan hash before | Plan hash after |
| --- | --- | ---: | --- | ---: | ---: | ---: | ---: | --- | --- |
${speedRows}
