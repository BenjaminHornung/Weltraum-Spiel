...
const command = (
  ...
  source: "source.test",
  sequence: 1,
  budgets: { maxVisitedBricks: 8, maxVisitedCells: 64, maxSelectedCells: 64, maxChangedCells: 64, maxConnectivityCells: 64, maxConnectivityFacts: 64, maxComponents: 8, maxMassCells: 64 },
  ...(kind === "SubtractBox"
...
describe("Structural Microvoxel persistence", () => {
  it("rejects persistence above 16 MiB UTF-8 before JSON.parse on decode and before decode on encode", () => {
    const oversizedUtf8 = "é".repeat(Math.floor(STRUCTURAL_MAX_PERSISTENCE_UTF8_BYTES / 2) + 1);
    const parseSpy = vi.spyOn(JSON, "parse");
    try {
      expect(() => decodeStructuralObject(oversizedUtf8)).toThrow(StructuralValidationError);
      expect(() => encodeStructuralObject({
        ...objectFixture(),
        objectId: oversizedUtf8
      } as StructuralObject)).toThrow(StructuralValidationError);
  ...
  it("round-trips schema-versioned object and result bytes through an A-B-A derivation with revisions, hashes, and evidence intact", () => {
    ...
    const resultB = accepted(applyStructuralDestructionCommand(a, command(a, "SubtractBox", "command.persist")));
    ...
    expect(deriveStructuralComponentClassification(restoredA, { maxVisitedCells: 8, maxComponents: 8, maxIndexedFacts: 8 }).components.map((component) => component.componentId))
      .toEqual(deriveStructuralComponentClassification(a, { maxVisitedCells: 8, maxComponents: 8, maxIndexedFacts: 8 }).components.map((component) => component.componentId));
  ...
  it("accepts append-only evidence with descending unique command IDs and revalidates it after persistence round-trip", () => {
    ...
    const first = accepted(applyStructuralDestructionCommand(original, command(original, "SetMaterialSphere", "command-z")));
    const second = accepted(applyStructuralDestructionCommand(first.object, command(first.object, "SetMaterialSphere", "command-a", {
  ...
  it("caps persistence history before element traversal and before canonical hashing", () => {
    const object = objectFixture();
    let getterCalls = 0;
    const oversizedHistory = new Array(STRUCTURAL_MAX_COMMAND_EVIDENCE + 1);
    Object.defineProperty(oversizedHistory, "0", {
      enumerable: true,
      configurable: true,
      get: () => {
        getterCalls += 1;
        return object.commandEvidence[0];
      }
    });
    const forged = { ...object, commandEvidence: oversizedHistory } as StructuralObject;
    try {
      encodeStructuralObject(forged);
      throw new Error("Expected oversized history rejection.");
    } catch (error) {
      expect(error).toBeInstanceOf(StructuralValidationError);
      if (error instanceof StructuralValidationError) expect(error.path).toBe("object/commandEvidence");
    }
    expect(getterCalls).toBe(0);

    const projection = JSON.parse(encodeStructuralObject(object)) as Record<string, unknown>;
    projection.commandEvidence = new Array(STRUCTURAL_MAX_COMMAND_EVIDENCE + 1).fill(null);
    try {
      decodeStructuralObject(canonicalAdaptiveJson(projection));
      throw new Error("Expected persisted oversized history rejection.");
    } catch (error) {
      expect(error).toBeInstanceOf(StructuralValidationError);
      if (error instanceof StructuralValidationError) expect(error.path).toBe("object/commandEvidence");
    }
  ...
  it("fails closed for persistence tamper, noncanonical bytes, unknown keys, and unknown versions", () => {
    ...
    const cases = [
      canonicalAdaptiveJson({ ...projection, contentHash: "fnv1a64-v1:0000000000000501" }),
    ...
    const result = accepted(applyStructuralDestructionCommand(object, command(object, "SubtractBox", "command.result")));
    const encodedResult = encodeStructuralResult(result);
    const resultProjection = JSON.parse(encodedResult) as Record<string, unknown>;
    expect(() => decodeStructuralResult(canonicalAdaptiveJson({ ...resultProjection, resultHash: "fnv1a64-v1:0000000000000501" }))).toThrow(StructuralValidationError);
  ...
  it("repeats equal inputs byte-for-byte and canonicalizes permitted brick and material permutations without duplicating product algorithms", () => {
    ...
    const firstResult = accepted(applyStructuralDestructionCommand(first, command(first, "SetMaterialSphere", "command.repeat")));
    const secondResult = accepted(applyStructuralDestructionCommand(second, command(second, "SetMaterialSphere", "command.repeat")));
