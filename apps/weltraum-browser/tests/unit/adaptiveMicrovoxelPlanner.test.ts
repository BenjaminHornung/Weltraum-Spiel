...
import {
  ...
  ADAPTIVE_MAX_ACTIVE_COVERAGE_ENTRIES,
  ADAPTIVE_MAX_PLAN_AGGREGATE_ENTRIES,
  ADAPTIVE_MAX_PLAN_ARRAY_ENTRIES,
  ADAPTIVE_MAX_PLANNER_ENUMERATION_WORK,
...
describe("adaptive microvoxel planner and residency obligations 13-20", () => {
  it("[13] produces the same canonical plan for equal snapshots regardless of caller order and preserves inputs", () => {
    ...
    expect(validateAdaptivePlanResult(first)).toEqual(first);
    expect(isDeepFrozen(validateAdaptivePlanResult(first))).toBe(true);
  });

  it("rejects resident, refinement, proof, and residency accessors without invoking caller getters", () => {
    const baseSnapshot = snapshot([], []);
    for (const field of ["resident", "refinementRequests"] as const) {
      let getterCalls = 0;
      ...
      expect(() => createAdaptivePlannerSnapshotProjection({
        ...baseSnapshot,
        [field]: accessorValues
      } as AdaptivePlannerSnapshot)).toThrow(AdaptiveAuthorityError);
      expect(getterCalls).toBe(0);
    }

    let proofGetterCalls = 0;
    const proofBricks: ReturnType<typeof materializeAdaptiveBrick>[] = [];
    Object.defineProperty(proofBricks, "0", {
      enumerable: true,
      configurable: true,
      get: () => {
        proofGetterCalls += 1;
        return brickFor(parent);
      }
    });
    expect(() => createAdaptiveResidentValidationProofs({
      bricks: proofBricks,
      brickRevision: plannerBrickRevision,
      snapshot: {} as AdaptivePlannerSnapshot
    })).toThrow(AdaptiveAuthorityError);
    expect(proofGetterCalls).toBe(0);

    const authority = createAdaptiveAuthorityRetention({ baseField, editJournal: planningJournal });
    let residencyGetterCalls = 0;
    const residentKeys: AdaptiveBrickKey[] = [];
    Object.defineProperty(residentKeys, "0", {
      enumerable: true,
      configurable: true,
      get: () => {
        residencyGetterCalls += 1;
        return parent;
      }
    });
    expect(() => releaseAdaptiveResidency(residentKeys, [], "evict", authority)).toThrow(AdaptiveAuthorityError);
    expect(residencyGetterCalls).toBe(0);
  });

  it("uses plain descriptor copies for snapshot, proof, and residency arrays with foreign prototypes", () => {
    const residentSnapshot = snapshot([resident(parent)], []);
    let residentMapCalls = 0;
    const inheritedResidents = withInheritedMapGetter([...residentSnapshot.resident], () => {
      residentMapCalls += 1;
    });
    const residentProjection = createAdaptivePlannerSnapshotProjection({
      ...residentSnapshot,
      resident: inheritedResidents
    });
    expect(residentProjection.resident).toHaveLength(1);
    expect(residentMapCalls).toBe(0);

    let refinementMapCalls = 0;
    const inheritedRefinements = withInheritedMapGetter(
      [requestForKey(key(4), "request.inherited-map")],
      () => {
        refinementMapCalls += 1;
      }
    );
    const refinementPlan = planAdaptiveMicrovoxels({
      snapshot: { ...snapshot([], []), refinementRequests: inheritedRefinements }
    });
    expect(refinementPlan.status).toBe("accepted");
    expect(refinementPlan.desired).toHaveLength(1);
    expect(refinementMapCalls).toBe(0);

    const prooflessSnapshot: AdaptivePlannerSnapshot = {
      ...residentSnapshot,
      resident: residentSnapshot.resident.map(({ validationProof: _proof, ...entry }) => entry)
    };
    let proofMapCalls = 0;
    const inheritedProofBricks = withInheritedMapGetter([brickFor(parent)], () => {
      proofMapCalls += 1;
    });
    const proofs = createAdaptiveResidentValidationProofs({
      bricks: inheritedProofBricks,
      brickRevision: plannerBrickRevision,
      snapshot: prooflessSnapshot
    });
    expect(proofs).toHaveLength(1);
    expect(proofMapCalls).toBe(0);

    const authority = createAdaptiveAuthorityRetention({ baseField, editJournal: planningJournal });
    let residentReleaseMapCalls = 0;
    let releasedMapCalls = 0;
    const inheritedResidentKeys = withInheritedMapGetter([parent], () => {
      residentReleaseMapCalls += 1;
    });
    const inheritedReleasedKeys = withInheritedMapGetter([parent], () => {
      releasedMapCalls += 1;
    });
    const release = releaseAdaptiveResidency(inheritedResidentKeys, inheritedReleasedKeys, "evict", authority);
    expect(release.residentKeys).toEqual([]);
    expect(release.releasedKeys).toEqual([parent]);
    expect(residentReleaseMapCalls).toBe(0);
    expect(releasedMapCalls).toBe(0);
  });

  it("rejects oversized proof batches before snapshot validation without mutating caller input", () => {
    const sentinel = brickFor(parent);
    const oversized = new Array(ADAPTIVE_MAX_RESIDENT_SUMMARIES + 1).fill(sentinel);
    ...
    expect(rejected).toBeInstanceOf(AdaptiveAuthorityError);
    expect((rejected as AdaptiveAuthorityError).path).toBe("proofs/bricks");
    expect(oversized).toHaveLength(ADAPTIVE_MAX_RESIDENT_SUMMARIES + 1);
    expect(oversized.every((entry) => entry === sentinel)).toBe(true);
  });

  it("rejects oversized and aggregate plan arrays before nested validation without mutating inputs", () => {
    const valid = planAdaptiveMicrovoxels({ snapshot: snapshot([], []) });
    const sentinelKey = key(4);
    const oversized = new Array(ADAPTIVE_MAX_PLAN_ARRAY_ENTRIES + 1).fill(sentinelKey);
    expect(() => validateAdaptivePlanResult({ ...valid, desired: oversized } as never))
      .toThrow(AdaptiveAuthorityError);
    expect(oversized.every((entry) => entry === sentinelKey)).toBe(true);

    const requiredChildren = new Array(ADAPTIVE_MAX_PLAN_ARRAY_ENTRIES).fill(sentinelKey);
    const fallbackEntry = { ancestor: sentinelKey, requiredChildren, coverage: quantumBoundsForKey(sentinelKey) };
    const fallbackCount = Math.floor(ADAPTIVE_MAX_PLAN_AGGREGATE_ENTRIES / ADAPTIVE_MAX_PLAN_ARRAY_ENTRIES) + 1;
    const aggregateFallback = new Array(fallbackCount).fill(fallbackEntry);
    let aggregateRejected: unknown;
    try {
      validateAdaptivePlanResult({ ...valid, fallback: aggregateFallback } as never);
    } catch (error) {
      aggregateRejected = error;
    }
    expect(aggregateRejected).toBeInstanceOf(AdaptiveAuthorityError);
    expect((aggregateRejected as AdaptiveAuthorityError).path).toContain("requiredChildren");
    expect(aggregateFallback.every((entry) => entry === fallbackEntry)).toBe(true);
    expect(requiredChildren.every((entry) => entry === sentinelKey)).toBe(true);
  });

  it("rejects oversized planner arrays before copying or sorting and preserves caller inputs", () => {
    const base = snapshot([], []);
    const cases = [
      ["resident", ADAPTIVE_MAX_RESIDENT_SUMMARIES],
      ["activeCoverage", ADAPTIVE_MAX_ACTIVE_COVERAGE_ENTRIES],
      ["refinementRequests", ADAPTIVE_MAX_REFINEMENT_REQUESTS]
    ] as const;
    for (const [field, limit] of cases) {
      const sentinel = {};
      const oversized = new Array(limit + 1).fill(sentinel);
      expect(() => createAdaptivePlannerSnapshotProjection({ ...base, [field]: oversized } as AdaptivePlannerSnapshot))
        .toThrow(AdaptiveAuthorityError);
  ...
  it("rejects more than 4096 target bricks through bounded enumeration without a partial plan", () => {
    ...
    expect(result).toMatchObject({
      status: "rejected",
      budget: "work",
      required: ADAPTIVE_MAX_PLANNER_ENUMERATION_WORK + 1,
      limit: ADAPTIVE_MAX_PLANNER_ENUMERATION_WORK
    });
    expect(result.desired).toEqual([]);
    expect(result.keep).toEqual([]);
    expect(result.materialize).toEqual([]);
