...
}: { bricks?: readonly BrickSeed[] } = {}): StructuralObject => {
...
describe("Structural Microvoxel commands", () => {
  it("applies all four commands with exact sphere centers, half-open box bounds, local translation, metadata preservation, canonical ordering, and validated publication", () => {
    ...
    const subtractSphere = accepted(applyStructuralDestructionCommand(original, commandInput("SubtractSphere", globalSphere())));
    ...
    expect(accepted(applyStructuralDestructionCommand(original, commandInput("SubtractBox", localHalfOpen))).object.bricks[0].cells.map((cell) => cell.localIndex)).toEqual([2]);

    for (const [kind, shape] of [["SetMaterialSphere", globalSphere()], ["SetMaterialBox", globalBox()]] as const) {
      const result = accepted(applyStructuralDestructionCommand(original, commandInput(kind, shape)));
    ...
    const changed = accepted(applyStructuralDestructionCommand(acrossBricks, commandInput("SubtractBox", globalBox(
  ...
  it("honors material filters and skips non-destructible occupied material", () => {
    ...
    const filtered = accepted(applyStructuralDestructionCommand(mixed, commandInput("SubtractBox", globalBox(
    ...
    const skipped = accepted(applyStructuralDestructionCommand(nonDestructible, commandInput("SubtractBox", globalBox(
  ...
  it("rejects CAS, target authority, coverage, budget, and duplicate failures atomically", () => {
    const original = objectFixture();
    const cases = [
      commandInput("SubtractBox", globalBox(), { expectedObjectRevision: 1, resultingObjectRevision: 2, commandId: "command.cas" }),
      commandInput("SubtractBox", globalBox(), { targetObjectId: "object.other", commandId: "command.target" }),
      commandInput("SubtractBox", globalBox({ x: 16, y: 0, z: 0 }, { x: 17, y: 1, z: 1 }), { commandId: "command.coverage" }),
      commandInput("SubtractBox", globalBox(), { commandId: "command.budget", budgets: { ...commandBudgets, maxVisitedCells: 1 } })
    ];
    ...
    expect(results.map((result) => result.status === "Rejected" ? result.code : null)).toEqual([
      "RevisionConflict", "WrongTarget", "MissingBrickCoverage", "BudgetExceeded"
    ]);
    ...
    const noChange = accepted(applyStructuralDestructionCommand(original, commandInput("SetMaterialBox", globalBox(), { commandId: "command.100", materialId: 1 })));
    const duplicate = applyStructuralDestructionCommand(noChange.object, commandInput("SetMaterialBox", globalBox(), {
  ...
  it("rejects every uncovered command budget dimension and arithmetic overflow fail-closed", () => {
    ...
    const cases: readonly Readonly<{
      ...
      {
        object: acrossBricks,
        result: applyStructuralDestructionCommand(acrossBricks, commandInput("SubtractBox", globalBox(
          { x: 15, y: 0, z: 0 }, { x: 17, y: 1, z: 1 }
      ...
      {
        object: original,
        result: applyStructuralDestructionCommand(original, commandInput("SubtractBox", globalBox(
          { x: 0, y: 0, z: 0 }, { x: 3, y: 1, z: 1 }
      ...
      {
        object: original,
        result: applyStructuralDestructionCommand(original, commandInput("SubtractSphere", globalSphere(
  ...
  it("distinguishes accepted NoChange from mutation revisions, hashes, evidence, invalidations, and immutable old state", () => {
    const original = objectFixture();
    const noChange = accepted(applyStructuralDestructionCommand(original, commandInput("SetMaterialBox", globalBox({ x: 5, y: 0, z: 0 }, { x: 6, y: 1, z: 1 }), {
    ...
    const mutation = accepted(applyStructuralDestructionCommand(original, commandInput("SubtractBox", globalBox(), { commandId: "command.mutation" })));
