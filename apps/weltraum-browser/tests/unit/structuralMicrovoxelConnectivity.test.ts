...
const objectFixture = (
  ...
  const canonicalCellSeeds = bricks
    .flatMap((brick) => brick.cells.map((cell) => ({ key: brick.key, cell })))
    .sort((left, right) =>
      left.key.originQuantum.x - right.key.originQuantum.x ||
      left.key.originQuantum.y - right.key.originQuantum.y ||
      left.key.originQuantum.z - right.key.originQuantum.z ||
      left.cell.x - right.cell.x ||
      (left.cell.y ?? 0) - (right.cell.y ?? 0) ||
      (left.cell.z ?? 0) - (right.cell.z ?? 0));
  const records: AdaptiveEditInput[] = canonicalCellSeeds.map(({ key, cell }, index) => {
    const min = { x: key.originQuantum.x + cell.x, y: cell.y ?? 0, z: cell.z ?? 0 };
    return {
...
describe("Structural Microvoxel connectivity", () => {
  it("uses six neighbors across brick seams, separates diagonals, classifies anchors, and keeps non-connecting inactive Joint endpoints", () => {
    ...
    const classification = deriveStructuralComponentClassification(object, { maxVisitedCells: 8, maxComponents: 4, maxIndexedFacts: 16 });
    ...
    expect(classification.fragments[0]).toMatchObject({
      componentId: diagonalComponent?.componentId,
      objectId: object.objectId,
      objectRevision: object.objectRevision,
      sourceContentHash: object.contentHash
    });
    expect(classification.fragments[0].occupiedCells.map(serializeStructuralCellAddress)).toEqual([serializeStructuralCellAddress(diagonal)]);
    expect(Object.isFrozen(classification.fragments[0])).toBe(true);
    expect(() => deriveStructuralComponentClassification(object, {
      maxVisitedCells: 8,
      maxComponents: 4,
      maxIndexedFacts: 5
    })).toThrowError(/fact budget/);
    ...
    const reactivatedClassification = deriveStructuralComponentClassification(reactivated, { maxVisitedCells: 8, maxComponents: 4, maxIndexedFacts: 16 });
    const reactivatedRepeatClassification = deriveStructuralComponentClassification(reactivatedRepeat, { maxVisitedCells: 8, maxComponents: 4, maxIndexedFacts: 16 });
  ...
  it("canonicalizes traversal and component IDs while binding IDs to objectRevision", () => {
    ...
    const noChangeCommand = validateStructuralDestructionCommand({
      ...
      expectedAdaptiveSource: first.source,
      budgets: { maxVisitedBricks: 8, maxVisitedCells: 8, maxSelectedCells: 8, maxChangedCells: 8, maxConnectivityCells: 8, maxConnectivityFacts: 8, maxComponents: 8, maxMassCells: 8 },
    ...
    const budgets = { maxVisitedCells: 8, maxComponents: 8, maxIndexedFacts: 8 };
    const firstClassification = deriveStructuralComponentClassification(first, budgets);
    const permutedClassification = deriveStructuralComponentClassification(permuted, budgets);
    const revisedClassification = deriveStructuralComponentClassification(revised, budgets);
    const firstComponents = firstClassification.components;
    const permutedComponents = permutedClassification.components;
    const revisedComponents = revisedClassification.components;
    ...
    expect(permutedClassification.fragments.map((fragment) => fragment.fragmentId))
      .toEqual(firstClassification.fragments.map((fragment) => fragment.fragmentId));
    ...
    expect(revisedClassification.fragments.map((fragment) => fragment.fragmentId))
      .not.toEqual(firstClassification.fragments.map((fragment) => fragment.fragmentId));
    expect(firstComponents.map((component) => component.componentId)).toEqual(
      [...firstComponents].map((component) => component.componentId).sort((left, right) => left < right ? -1 : left > right ? 1 : 0)
    );
  });
});
