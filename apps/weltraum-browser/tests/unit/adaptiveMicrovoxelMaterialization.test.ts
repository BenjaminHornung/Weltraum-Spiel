...
describe("adaptive microvoxel materialization obligations 10-12", () => {
  ...
  it("applies sphere edits to half-open cells with exact negative and large-safe integer arithmetic", () => {
    ...
    const precisionOutside = materializeAdaptiveBrick({ key: key(), baseField: base(), editJournal: createAdaptiveEditJournal([{
      editId: "edit.precision-outside", sequence: 1, expectedRegionRevision: 0, resultRegionRevision: 1,
      actorId: "actor.test", sourceId: "tool.test", operation: "SubtractSphere",
      sphere: {
        center: { x: -Number.MAX_SAFE_INTEGER, y: -1, z: 0 },
        radiusQuantum: Number.MAX_SAFE_INTEGER
      }
    }]) });
    expect(precisionOutside.occupancy.every((value) => value === 1)).toBe(true);
  });

  it("rejects materialized channel accessors without invoking caller getters", () => {
    const brick = materializeAdaptiveBrick({ key: key(), baseField: base(), editJournal: createAdaptiveEditJournal([]) });
    let getterCalls = 0;
    const density = [...brick.density];
    Object.defineProperty(density, "0", {
      enumerable: true,
      configurable: true,
      get: () => {
        getterCalls += 1;
        return brick.density[0];
      }
    });
    expect(() => validateMaterializedAdaptiveBrick({ ...brick, density } as typeof brick)).toThrow(AdaptiveAuthorityError);
    expect(getterCalls).toBe(0);
  });

  it("copies materialized channel descriptors without inherited map access", () => {
    const brick = materializeAdaptiveBrick({ key: key(), baseField: base(), editJournal: createAdaptiveEditJournal([]) });
    let inheritedMapCalls = 0;
    const density = withInheritedMapGetter([...brick.density], () => {
      inheritedMapCalls += 1;
    });
    const callerPrototype = Object.getPrototypeOf(density);
    const validated = validateMaterializedAdaptiveBrick({ ...brick, density } as typeof brick);
    expect(validated.contentHash).toBe(brick.contentHash);
    expect(validated.density).toEqual(brick.density);
    expect(inheritedMapCalls).toBe(0);
    expect(Object.getPrototypeOf(density)).toBe(callerPrototype);
  });
