...
import {prepareHvpRigidBody} from "../physics/rigidRecipe";
...
export interface HvpTerrainFragment {
  ...
  readonly affectedLeaves:readonly string[];
}
...
export const analyzeHvpSupportSnapshot=(source:HvpCellReader,changed:readonly HvpCell[],budgets:{maxProbes?:number;maxFragmentCells?:number}={}):HvpSupportReport=>{
  ...
  try{
    ...
    for(const seed of [...seeds].sort((a,b)=>a-b)){
      ...
      let massKg:number,colliders:number;
      try{
        const recipe=prepareHvpRigidBody(ingestHvpStructuralCells(`hvp-terrain-fragment-${digest}`,cells,materials));
      ...
      fragments.push(Object.freeze({id:`hvp-terrain-fragment-${digest}`,digest,cells,min,max,massKg,colliders,affectedLeaves:Object.freeze(leaves)}));
    }
    return finish("Ready");
