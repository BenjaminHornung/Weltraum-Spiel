...
export const ingestHvpStructuralCells = (id:string, input:readonly HvpStructuralCell[], materialInput:readonly unknown[],
  ...
  return measure("ingestTotalMs",()=>{
    ...
    const {cells,origins,runs}=measure("ingestSortRunsMs",()=>{
    ...
    const {frame,baseField,editJournal}=measure("ingestJournalMs",()=>{
    ...
    const bricks=measure("ingestMaterializeMs",()=>[...origins.values()].sort((a,b)=>a.z-b.z||a.y-b.y||a.x-b.x).map(originQuantum=>materializeAdaptiveBrick({
    ...
    const {snapshot,resident,proofs}=measure("ingestProofsMs",()=>{
    ...
    return measure("ingestStructuralMs",()=>{
