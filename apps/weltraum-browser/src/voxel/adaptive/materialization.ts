...
const validateDenseChannel = (value: unknown, name: string): readonly unknown[] => {
  if (!Array.isArray(value) || value.length !== ADAPTIVE_BRICK_CELL_COUNT) {
    return fail("InvalidBaseField", `brick/${name}`, "Materialized channels must contain exactly 4096 dense cells.");
  }
  for (let index = 0; index < value.length; index += 1) {
    if (!Object.hasOwn(value, index)) return fail("InvalidBaseField", `brick/${name}/${index}`, "Sparse channels are rejected.");
  }
  return value;
};
...
const overlapsSphere = (cell: QuantumBounds, edit: AdaptiveEditRecord): boolean => {
  ...
  for (const axis of ["x", "y", "z"] as const) {
    ...
    const delta = BigInt(coordinate) - BigInt(nearest);
    distanceSquared += delta * delta;
  }
  const radius = BigInt(sphere.radiusQuantum);
  const radiusSquared = radius * radius;
  return distanceSquared < radiusSquared || (distanceSquared === radiusSquared && tangentPointIsIncluded);
};
