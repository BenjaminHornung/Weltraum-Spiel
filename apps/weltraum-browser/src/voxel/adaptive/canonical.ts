...
const canonicalize = (value: unknown, path: string, ancestors: WeakSet<object>): AdaptiveCanonicalValue => {
  ...
  if (Array.isArray(value)) {
    ...
    if (
      ...
      return fail("InvalidCanonicalValue", path, "Canonical arrays may contain only dense indexed elements.");
    }
    for (let index = 0; index < value.length; index += 1) {
      if (!Object.hasOwn(value, index)) return fail("InvalidCanonicalValue", `${path}/${index}`, "Sparse arrays are not canonical.");
      const descriptor = Object.getOwnPropertyDescriptor(value, String(index));
      if (descriptor === undefined || !descriptor.enumerable || !("value" in descriptor)) {
        return fail("InvalidCanonicalValue", `${path}/${index}`, "Canonical array elements must be enumerable data properties.");
      }
    }
    ancestors.add(value);
...
const requireDenseArray = (value: unknown, path: string): readonly unknown[] => {
  if (!Array.isArray(value)) return fail("InvalidPlannerInput", path, "Expected a dense array.");
  for (let index = 0; index < value.length; index += 1) {
    if (!Object.hasOwn(value, index)) return fail("InvalidPlannerInput", `${path}/${index}`, "Sparse arrays are rejected.");
  }
  return value;
};
...
export const createAdaptivePlannerSnapshotProjection = (snapshot: AdaptivePlannerSnapshot) => {
  ...
  const residents = requireDenseArray(record.resident, "snapshot/resident").map((value, index) => {
  ...
  const activeCoverage = requireDenseArray(record.activeCoverage, "snapshot/activeCoverage").map((value, index) => {
  ...
  }).sort((left, right) => canonicalAdaptiveJson(left).localeCompare(canonicalAdaptiveJson(right)));

  const refinementRequests = requireDenseArray(record.refinementRequests, "snapshot/refinementRequests")
...
export const validateAdaptivePlannerSnapshotSemantics = (
  ...
  const refinementRequests = snapshot.refinementRequests.map(validateRefinementRequest);
...
export const createAdaptiveResidentValidationProofs = ({ bricks, brickRevision, snapshot }: CreateAdaptiveResidentValidationProofsInput): readonly AdaptiveResidentValidationProof[] => {
  const { projection, snapshotProjectionDigest } = validateAdaptivePlannerSnapshotSemantics(snapshot, { allowProoflessReadyResidents: true });
  return deepFreeze(bricks.map((brick) => issueAdaptiveResidentValidationProof(brick, brickRevision, projection, snapshotProjectionDigest)));
};
