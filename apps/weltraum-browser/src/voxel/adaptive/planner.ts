...
const uniqueCoverageVolume = (desired: readonly DesiredCandidate[]): number => {
  ...
  for (const candidate of [...desired].sort((left, right) => left.key.level - right.key.level || compareAdaptiveBrickKeys(left.key, right.key))) {
    ...
    for (const accepted of union) fragments = fragments.flatMap((fragment) => subtractBounds(fragment, accepted));
    union.push(...fragments);
  }
  return union.reduce((sum, bounds) => safeSum(sum, boundsVolume(bounds, "budgets/maxCoverageQuantum"), "budgets/maxCoverageQuantum"), 0);
};
...
export const planAdaptiveMicrovoxels = ({ snapshot }: PlanAdaptiveMicrovoxelsInput): AdaptivePlanResult => {
  ...
  if (validatedRequests.some((entry) => entry.count > ADAPTIVE_MAX_DESIRED_BRICKS)) return rejectBudget("brick-count", ADAPTIVE_MAX_DESIRED_BRICKS + 1, ADAPTIVE_MAX_DESIRED_BRICKS, snapshotProjectionDigest);
