...
export const createAdaptiveEditJournal = (
  ...
  const inputRecords = requireDenseDataPropertyArray(inputs, "records", "InvalidEditJournal", {
    maximumLength: ADAPTIVE_MAX_JOURNAL_RECORDS
  }) as readonly (AdaptiveEditInput | AdaptiveEditRecord)[];
...
export const validateAdaptiveEditJournal = (journal: AdaptiveEditJournal): AdaptiveEditJournal => {
  ...
  const journalRecords = requireDenseDataPropertyArray(
    record.records,
    "journal/records",
    "InvalidEditJournal",
    { maximumLength: ADAPTIVE_MAX_JOURNAL_RECORDS }
  ) as readonly AdaptiveEditRecord[];
  const rebuilt = createAdaptiveEditJournal(journalRecords, record.initialRegionRevision as number);
