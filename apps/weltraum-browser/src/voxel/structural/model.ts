...
export const createStructuralMaterialTable = (values: readonly unknown[]): readonly StructuralMaterialDefinition[] => {
  const materials = structuralDenseArray(values, "materials", STRUCTURAL_MAX_MATERIAL_DEFINITIONS)
...
export const createStructuralBrick = (
  ...
  const cells = structuralDenseArray(record.cells, `${path}/cells`, STRUCTURAL_MAX_BRICK_CELLS).map((entry, index): StructuralBrickCell => {
...
export const validateStructuralAdaptiveSourceBinding = (value: unknown, path = "source"): StructuralAdaptiveSourceBinding => {
  ...
  const proofDigests = structuralDenseArray(record.proofDigests, `${path}/proofDigests`, STRUCTURAL_MAX_PROOF_DIGESTS).map((digest, index) => requireStructuralHash(digest, `${path}/proofDigests/${index}`));
...
export const reconstructStructuralObjectInternal = (value: unknown): StructuralObject => {
  ...
  const bricks = structuralDenseArray(input.bricks, "bricks", STRUCTURAL_MAX_BRICKS).map((brick, index) => createStructuralBrick(brick, materials, frame, `bricks/${index}`)).sort((left, right) => compareAdaptiveBrickKeys(left.key, right.key));
  ...
  const anchors = structuralDenseArray(input.anchors, "anchors", STRUCTURAL_MAX_ANCHORS).map((anchor, index) => validateAnchor(anchor, frame, `anchors/${index}`)).sort((left, right) => compareStrings(left.anchorId, right.anchorId));
  const joints = structuralDenseArray(input.joints, "joints", STRUCTURAL_MAX_JOINTS).map((joint, index) => validateJoint(joint, frame, `joints/${index}`)).sort((left, right) => compareStrings(left.jointId, right.jointId));
...
const validateMaterialBindings = (
  ...
  const knownMaterials = materialIds(materials);
  const entries = structuralDenseArray(values, "materialBindings", STRUCTURAL_MAX_MATERIAL_BINDINGS).map((value, index): StructuralAdaptiveMaterialBinding => {
...
export const createStructuralObjectFromAdaptive = (value: unknown): StructuralObject => {
  ...
  const adaptiveBricks = structuralDenseArray(input.bricks, "ingest/bricks", STRUCTURAL_MAX_BRICKS).map((brick) => validateMaterializedAdaptiveBrick(brick as MaterializedAdaptiveBrick));
