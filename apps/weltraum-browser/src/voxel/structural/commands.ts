...
export const applyStructuralDestructionCommand = (
  ...
  try {
    ...
    const commandEvidence = structuralDenseArray(object.commandEvidence, "object/commandEvidence", STRUCTURAL_MAX_COMMAND_EVIDENCE);
    ...
    if (commandEvidence.some((entry) => (entry as StructuralCommandEvidence).commandId === command.commandId)) throw new StructuralCommandError("DuplicateCommand", "command/commandId", "Command ID was already consumed.");
