...
const objectFromProjection = (value: unknown): StructuralObject => {
  ...
  const bricks = structuralDenseArray(record.bricks, "object/bricks", STRUCTURAL_MAX_BRICKS).map((value, index) => {
  ...
  const anchors = structuralDenseArray(record.anchors, "object/anchors", STRUCTURAL_MAX_ANCHORS).map((value, index) => {
  ...
  const joints = structuralDenseArray(record.joints, "object/joints", STRUCTURAL_MAX_JOINTS).map((value, index) => {
  ...
  const commandEvidence = structuralDenseArray(record.commandEvidence, "object/commandEvidence", STRUCTURAL_MAX_COMMAND_EVIDENCE).map((value, index) => {
    ...
    return {
      ...evidence,
      changedBrickKeys: structuralDenseArray(evidence.changedBrickKeys, `${path}/changedBrickKeys`, STRUCTURAL_MAX_CHANGED_BRICK_KEYS)
...
const resultFromProjection = (value: unknown): StructuralCommandResult => {
  ...
  if (!accepted) {
    ...
    const changedBrickKeys = structuralDenseArray(record.changedBrickKeys, "result/changedBrickKeys", STRUCTURAL_MAX_CHANGED_BRICK_KEYS)
    ...
    const invalidations = structuralDenseArray(record.invalidations, "result/invalidations", STRUCTURAL_MAX_INVALIDATIONS)
