...
export type DecodeGraphicsSettingsResult =
  | { readonly ok: true; readonly value: GraphicsSettingsV1 }
...
export function validateGraphicsSettings(value: unknown): value is GraphicsSettingsV1 {
  if (!isRecord(value) || !hasExactKeys(value, ["qualityPreset", "display", "shadows", "textures", "lighting", "effects", "antiAliasing", "vsync"])) {
    return false;
  }
...
export function normalizeGraphicsSettings(settings: GraphicsSettingsV1): GraphicsSettingsV1 {
  const cloned = structuredClone(settings);
  const normalized: GraphicsSettingsV1 = {
    ...cloned,
    qualityPreset: inferQualityPreset(cloned)
  };
  return cloneAndFreeze(normalized) as GraphicsSettingsV1;
}
...
export function createGraphicsSettingsEnvelope(settings: GraphicsSettingsV1): GraphicsSettingsEnvelopeV1 {
