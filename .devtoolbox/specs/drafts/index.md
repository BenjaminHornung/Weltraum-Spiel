...
These documents are intentionally stored under `.devtoolbox/specs/drafts/` so they do not look like active validated changes. A future Codex/DevToolbox run should promote one draft at a time into `.devtoolbox/specs/changes/<changeName>/` with `proposal.md`, `design.md`, `specs/<capability>/spec.md`, and `tasks.md`, then run DevToolbox validation.
