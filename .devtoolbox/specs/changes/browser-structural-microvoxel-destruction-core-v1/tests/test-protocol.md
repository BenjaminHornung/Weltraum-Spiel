...
`.github/**`, `infra/**`, and foreign changes. No package or E2E-group edit is
allowed.
...
3. exact materials, integer frame translation, source/proof binding, and
   caller-copy/recursive-freeze safety;
...
6. cell mass, removed-mass difference, COM/AABB, full symmetric inertia, and
   empty/finite/symmetry semantics;
...
Use the existing Playwright/Vite setup without modifying package, group, or
...
2. independent `reviewer-glm` for public-contract and rollback/data-loss risk.

Fix only confirmed in-scope findings, then rerun affected reviews and fresh
verification. Use the single DevToolbox execution for verification and
Completion Preflight; tasks remain open without evidence. After all tasks are
verified, call `plannotator_final_review` exactly once for the Structural
worktree root. If human feedback requests changes, repeat affected review and
verification and reopen final review for that root.
...
- any forbidden path, package/lock/config change, secret hit, API/authority
  scan finding, or diff-check failure appears;
