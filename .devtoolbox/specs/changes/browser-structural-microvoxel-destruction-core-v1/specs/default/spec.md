...
package/lock/Vite/Playwright files, `.github/**`, `infra/**`, and foreign
agent changes. The E2E spec is not added to a package test group.
...
object revisions, quantized global or object-local shape, optional canonical
...
hash of object ID, object revision, smallest canonical occupied-cell key, and
component content hash. Array/traversal order is not identity. A-B-A means
rehydrating the same persisted A revision; a later equal geometry with a new
revision may have new IDs.
...
tensor values, occupied count, source revision, and content hash. Empty state
...
closed. Mass is never derived from mesh output.
