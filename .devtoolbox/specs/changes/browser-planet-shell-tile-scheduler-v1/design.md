...
`planetPresentationAdapter.ts` is the sole load-execution and presentation boundary. It converts generated planet geometry into the existing `MeshArtifact` factory, translates exactly the explicit core `loadRequests` into jobs, and infers no load work from visibility lists. Hash/content identity is derived deterministically from stable tile/generator inputs. Presentation and Three backend source files remain untouched.

The adapter is stateless and returns a discriminated decision. It returns `publish` only when the readiness revision exactly matches the core plan, `coverageStatus` is `READY`, and every unique active key in core `primary ∪ fallback` is render-ready in that snapshot. A publish decision flattens the complete active set into presentation `visibleRepresentationKeys`, with each key exactly once and `fallbackRepresentationKeys` empty. On revision mismatch, `NOT_READY`, or any missing/non-ready active key (including an inconsistent `READY` declaration), it returns `hold-last-complete-plan` with no `VisibilityPlan`, a closed reason code, deterministic missing active tile IDs, and unchanged load jobs. Revision mismatch treats all active keys as unverified/missing. The caller owns and retains the last complete plan; no previous plan or hidden mutable state enters the adapter. A future caller may map hold to an explicit planet impostor, but V1 does not implement one.
...
Required states are far orbit, near orbit with parent fallback, child-ready handoff, and eviction/reactivated parent. Every deterministic harness fixture is complete and therefore must receive `publish`; the harness checks the discriminant before dispatching `ApplyVisibilityPlan` and fails explicitly on an unexpected hold rather than applying an empty or partial plan. A real caller encountering hold keeps its previously applied complete plan. Browser listeners reject console/page/request/HTTP errors. Screenshot baselines use the three required names.
