...
- [ ] **Item 2:** Duplizierte Ternary in `PrototypeTrajectoryPlanner.BuildSegments` (`:985-987`) aufloesen; vorher Spec `add-autopilot-obstacle-avoidance-v1` gegenlesen, ob ein Avoidance-Unterschied beabsichtigt war; Avoidance-Tests muessen gruen bleiben.
...
## Performance
- [ ] **Item 5:** HUD-Radar-Ziele aus `PrototypeWaypointManager` statt `FindObjectsByType` pro Frame (`PrototypePlayerHud.cs:1925`); `Resources.FindObjectsOfTypeAll`-Fallback (`:1928`) entfernen; Fallback-Scan nur ohne Manager, 1-Hz-Gate.
...
- [ ] **Item 7:** `RcsThrusterController`/`MainThrusterBank` im Autopilot cachen (`:4741`, `:5296`, `:5343`), Invalidierung bei Controller-Wechsel in `ResolveReferences()`.
- [ ] Profiler-Evidence vorher/nachher (1 min Flug, Planner offen) unter `tests/` ablegen.

## Housekeeping
- [ ] **Item 8:** `Assets/_Recovery/` loeschen (Inhalt vorher kurz sichten); `.gitignore` ergaenzen: `.idea/`, `UpgradeLog.htm`, `Assets/_Recovery/`, `artifacts/`.
