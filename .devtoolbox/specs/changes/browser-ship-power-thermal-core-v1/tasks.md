...
- [x] 7. Integrate the published feature against current main and complete review gates
  ...
  - Guidance: retain current main content and original feature commits; rerun the reduced matrix after any main drift and exact-head review after any head change.
  - Required skills/MCPs: subagent-driven-development, devtoolbox-specs-execution, requesting-code-review, verification-before-completion, finishing-a-development-branch.
  - Verification: run the user-specified serial Node 22 matrix and all listed integration gates; link fresh evidence to this task before completion.
  - Report: starting main, feature head, merge base, final head, paths, inventories, all results/hashes, preflight, PR/review threads, and remaining risks.
  - Stop: no force push, no main merge, no unauthorized scope, no weakened assertion/timeout, and no completion toggle unless canProceed is true.
