...
fail closed. Invalid keys shall be rejected without clamping, wrapping,
normalizing, or switching to `BigInt`.
...
shall not be compacted lossily.
...
descriptor digest and ordered journal digest. Equal inputs shall reproduce
byte-identical values and deterministic FNV-1a64 V1 hashes; changed
...
and journal records only. It shall exclude timestamps, random values,
locale/object ordering, request/worker/cache metadata, timing, camera/render
state, and derived presentation buffers. Unknown fields, duplicate keys,
...
budgets. Derived proof/digest fields are excluded from this projection so it is
non-circular.
...
## Requirement: Deferred package integration

This change shall not alter package scripts, dependencies, lockfiles, bootstrap,
existing worker/streaming paths, Three.js/Presentation, Hestia surface-lab
runtime, flight, navigation, persistence, gameplay, Playwright configuration,
CI group membership, or other shared files.

### Scenario: Scope audit

Given the completed implementation and evidence, when the changed-path audit is
run, then only explicitly approved isolated authority/test/evidence paths and
these five artifacts are present; forbidden shared paths are untouched.
...
authority behavior through the normal `/` route without TestBridge authority or
package integration. It shall prove every L0-L4 relationship, negative
...
complete from artifact editing alone. The ordered gate is fresh evidence,
technical review, completion preflight, exactly one final human review for the
in-scope worktree, then separate user authorization before the exact commit
`#WELTRAUM-000 Add adaptive microvoxel authority core` and non-force push to
`origin/feature/browser-adaptive-microvoxel-authority-v1`. No PR, merge,
deployment, or force-push is allowed.
