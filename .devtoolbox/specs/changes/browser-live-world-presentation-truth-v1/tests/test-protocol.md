...
- navigation-map-to-presentation adapter behavior with bounded telemetry enrichment,
- exact map-backed world entity, residence, and LOD binding,
...
# 3) NavigationMapSnapshot truth reconciliation
...
mcp__servicerunner__.tasks_completion_preflight
mcp__servicerunner__.tasks_toggle
git commit -m "#WELTRAUM-000 Reconcile world presentation with navigation map truth"

# 4) Unit, build, and full app sanity
npm run test -- tests/unit/worldPresentation.test.ts
npm run test -- tests/unit/worldPresentationRenderer.test.ts
npm run test
npm run build

# 5) Playwright evidence set
...
mcp__servicerunner__.tasks_completion_preflight
mcp__servicerunner__.tasks_toggle
git commit -m "#WELTRAUM-000 Add live world presentation truth evidence"
npm run test:e2e -- tests/e2e/browser-navigation-map-world-truth.spec.ts
...
# 6) Evidence and regression checks
git diff --check

# 7) Final task verification (completion follows the audit below)
...
```powershell
# Reload tasks.md and use Task 5's current sourceLine.
mcp__servicerunner__.tasks_completion_preflight
mcp__servicerunner__.tasks_toggle
git commit -m "#WELTRAUM-000 Complete live world presentation truth verification"
```
...
1. Snapshot determinism
   ...
   - floating-origin frame or revision changes may change `renderFrameRevision` while keeping `frameId` and semantic signature unchanged.
2. Route + segment integrity
   - visible or blocked routes match the map route by plan hash, target ID, and exact segment order.
   - hidden stale routes without canonical map geometry have an empty segment list.
...
4. Obstacle truth/decor
   ...
   - eight landmark and six ambient low-poly entities are map-backed world entities, not decoration.
   - only the cinematic belt, planet, and starfield are renderer-only; `decorativeAsteroidCount` is exactly 150.
5. World entities
   - each entry exposes exact ID, absolute position, chunk ID, residence, render LOD, presentation key, and `Ambient` or `Landmark` role.
   - fourteen fixed slots match exact entity IDs each frame; missing, dormant, and `Culled` entities are zero-scaled.
   - deterministic rotation and configured scale remain stable for visible residents.
6. Renderer contract
...
7. Mode behavior
   ...
   - normal canvas attributes expose states and counts only.
