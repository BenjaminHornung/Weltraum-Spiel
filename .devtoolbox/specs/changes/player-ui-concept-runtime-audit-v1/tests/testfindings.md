...
- Screenshots `05` through `11` remain synthetic HUD snapshot renders in Unity and are retained as renderer-state evidence. Screenshots `12` through `18` and the later `31`/`32` evidence are live PlayMode captures driven through real Bootstrap-created runtime components; they are still automated RenderTexture captures, not a human manual playthrough recording.
