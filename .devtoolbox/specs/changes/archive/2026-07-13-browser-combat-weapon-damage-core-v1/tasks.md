...
Implementation evidence is complete. The user explicitly authorized manual task closure despite the unavailable DevToolbox completion preflight; this checklist records that documented exception without claiming that DevToolbox ran.

## Phase 1 — Combat domain and fire control

- [x] 1. Implement stable IDs, canonicalization, vectors/frames, immutable validation, public contracts, and fixtures under `apps/weltraum-browser/src/combat/**`.
...
- [x] 2. Implement Weapon runtime advancement, stable Target selection, fire permission, and accepted fire transitions.
...
- [x] 3. Implement Projectile fixed-step advancement, expiry, Beam rays, Sphere/AABB intersections, and explicit Hit Results.
...
- [x] 4. Implement Armor/Hull/Module damage, status/recoverability, semantic effects, and canonical events.
...
- [x] 5. Add the normal-route Playwright scenario, deterministic evidence files, and focused Browser documentation.
...
- [x] 6. Run focused and full regression verification, inspect evidence/diff, and audit the allowlist.
  ...
  - Verification: use the complete command block and scope audits in `tests/test-protocol.md`; record actual results there before closing this task under the documented manual override.
...
- [x] 7. Document the unavailable DevToolbox preflight, commit, and push the isolated feature branch.
  - Objective: close the evidence-backed local checklist under the explicit user override, document the unavailable preflight, and deliver the reviewed branch without merging to `main`.
  - Acceptance: the unavailable DevToolbox `verify_run` and `tasks_completion_preflight` remain truthfully recorded; commit title is `#WELTRAUM-000 Add browser combat weapon and damage core` with the prescribed concise body and one line per changed file; branch pushes to `origin/feature/browser-combat-weapon-damage-core-v1`; completion report contains base SHA, branch/commit, contracts, behavior, all test evidence, scope audit, limitations, and the continuing DevToolbox access blocker.
  - Verification: clean expected status after commit, remote branch confirmed, no merge to `main`.
  - Stop/escalate: never claim that DevToolbox preflight ran. Manual checklist closure is permitted only because the user explicitly authorized this documented exception.
