...
> Planning note: these tasks were planned for the future implementation branch after `browser-autopilot-long-range-testfield-v1` merges. Implementation, verification, and review are now complete; this file is retained as the completed task record.

## Phase 1: Add deterministic candidate and waypoint generation

- [x] Objective: Generate ordered waypoint candidates deterministically for multi-obstacle route planning.
...
- [x] Objective: Build a route that can chain around more than the first blocking obstacle.
  - Files/search targets: `apps/weltraum-browser/src/navigation/multiObstaclePlanner.ts`, `apps/weltraum-browser/src/navigation/planners.ts`.
...
- [x] Objective: Validate every segment against every relevant obstacle and reject impossible routes.
...
- [x] Objective: Keep scoring deterministic while allowing Safe/Balanced/Fast to change only route intent metadata.
...
- [x] Objective: Cover deterministic routing, budget limits, safe rejection, and planHash stability.
...
- [x] Objective: Record browser evidence for the new planner step.
...
- [x] Objective: Reclassify the scenario catalog so current stress cases are explicit and the new planner target is clear.
  - Files/search targets: `apps/weltraum-browser/src/world/autopilotProvingGroundCourses.ts`, `apps/weltraum-browser/src/test-harness/scenarioRunner.ts`, `docs/browser-mainline/implementation-analysis/browser-multi-obstacle-route-planner-v1-scenario-requirements.json`.
