...
For a focused direct Vitest run, use the explicit Node entry point: `node node_modules/vitest/vitest.mjs run tests/unit/spatialUniverseClock.test.ts`.
...
Execution validates schema and plan hash and launches `command_argv` as a list with `shell=False`, using the child environment and working directory. On Windows execution resolves only native `.COM` and `.EXE` programs. Explicit or discovered `.CMD` and `.BAT` targets are rejected before `Popen` with a constant sanitized anticipated exception, and an unresolved command is never passed through. For PATH lookup, balanced quotes around entries are stripped, empty and non-absolute child PATH entries are ignored, and each absolute PATH directory is checked in directory order before trying the native extension candidates within that directory. There is no current-working-directory or parent-process PATH fallback. A separator-containing relative argv resolves only beneath the child working directory; an absolute argv is accepted by the runtime resolver only when it names an existing native executable, although canonical plan validation may independently prohibit absolute commands. Resolution does not alter canonical argv, plan hash, run records, or summaries.

On Windows the resolved native root process starts suspended, is assigned to a per-attempt Win32 Job Object configured with kill-on-close, and is then resumed. Timeout and Ctrl-C call `TerminateJobObject`; normal handle closure removes surviving owned descendants. Create, assignment, resume, termination, and `CloseHandle` failures are surfaced as `InfrastructureFailure`. A failed close retains ownership long enough to attempt owned-job termination and a close retry, and the run is never reported successful. The implementation never enumerates or terminates unrelated PIDs.
