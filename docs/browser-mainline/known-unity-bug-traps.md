...
| Long-range matrix overclaim | A 1000m/2500m browser testfield can look like full navigation maturity while only proving local-space deterministic courses. | Long-range evidence must separate `Pass`, `KnownStress` and `ExpectedFail`, list the current planner limits, and avoid player-facing route-mode or UI redesign claims. | `autopilot-long-range-summary.json` records representative runtime rows, counts, longest tested distance, speed-profile comparison, fuel/clearance ranges, no-snap/no-zero/no-silent-replan confirmation and stable `planHash`. |
| Normal-runtime snap or idle-zero shortcut | Setting `position = target.position`, `position = segment.end`, waypoint `velocity = vec3()`, terminal velocity clamp/zero, or idle/cancel velocity zero can make tests pass while hiding impossible flight. | Normal runtime progresses by actual envelope entry/crossing and terminal-speed gates without mutating position or velocity. Completed/Holding clears the active route lock, preserves `completedPlanHash`, and continues station-keeping through `applyFlightControllerStep()`; idle/cancel preserve drift unless a future explicit brake/kill-momentum command is specified. | No-snap/no-zero unit tests cover waypoint progression, terminal arrival, holding integration, post-arrival new-route selection, cancel and idle drift; browser evidence shows autopilot terminal brake/capture/hold through actuation. |
| Render smoothing mutates truth | Presentation interpolation can accidentally write back into fixed-step simulation state, making the renderer/camera a hidden flight authority. | Fixed-step truth stays deterministic; render snapshots clone previous/current truth and interpolate a read-only presentation pose. Only ship visual and camera consume the interpolated pose; HUD, routes, targets, VFX and TestBridge use owner truth. | Unit tests mutate returned presentation snapshots and assert loop truth is unchanged; E2E render snapshots record truth vs rendered pose, interpolation alpha, fixed steps and camera smoothing alpha. |
...
7. Long-range evidence must not reclassify current multi-obstacle/corridor planner limits as production passes.
