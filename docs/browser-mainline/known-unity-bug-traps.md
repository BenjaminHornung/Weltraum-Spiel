...
| Missing sockets/targets default to root/zero | Fallbacks make visuals appear to work while gameplay binds to wrong points. Procedural browser ship descriptors can also hide future GLB marker/socket mismatches. | Missing/ambiguous markers or targets fail closed with visible diagnostics. Browser ship visual descriptor validation now covers hull/body, cockpit/front, main engine, at least four RCS markers, muzzle placeholder and camera anchor; production GLB adoption must prove authored marker/socket parity before replacing the procedural fallback. | Unit/render snapshot tests fail if required procedural descriptors disappear; GLB loader follow-up should prove marker/socket descriptor mapping before replacing the procedural fallback. |
