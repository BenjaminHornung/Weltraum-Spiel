...
| VFX from input state | Thruster flames or RCS puffs driven directly by keydown make manual input look correct while autopilot, SAS or control saturation state is wrong. | VFX consume actuator telemetry only: main flame from `mainThrustActive`, RCS puffs from `rcsTranslationActive`, `rcsRotationActive` and `sasCorrectionActive`. | E2E snapshots assert VFX state during manual translation and autopilot burn from render snapshots/telemetry, not raw input state. |
...
| Missing sockets/targets default to root/zero | Fallbacks make visuals appear to work while gameplay binds to wrong points. Procedural browser ship descriptors can also hide future GLB marker/socket mismatches. | Missing/ambiguous markers or targets fail closed with visible diagnostics. Production ship asset adoption must validate hull/body, cockpit/front, main engine, RCS markers, muzzle placeholder and camera anchor descriptors instead of assuming sockets exist. | Builder/ship binding tests later assert duplicate/missing marker failures; GLB loader follow-up should prove marker/socket descriptor mapping before replacing the procedural fallback. |
