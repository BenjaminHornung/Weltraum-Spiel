...
| Tests / evidence hints | Direct local arrival with final position equal to target position, obstacle avoidance, off-route divergence, locked hash preservation, explicit `replanRequired`, target-selection fail-closed behavior, plan serialization/hash determinism, waypoint/point target taxonomy, arrival-envelope evidence, structured planner rejection, invalid obstacle rejection and deterministic candidate scoring tests. |
...
| Browser-native intent | HUD renders read-only snapshots and player-facing ViewModels: mode, fuel/throttle/RCS/SAS, selected target, route preview/locked-route state, compact radar-style route contact, warnings, target state and next action. Debug/TestBridge surfaces stay labeled and separate from player UI. |
