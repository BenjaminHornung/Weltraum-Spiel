...
| Browser-native intent | Deterministic ship state now carries explicit `ShipMass`, `FuelState`, `AuthorityState`, `BrakingReserve` and owner `FlightSnapshot` contracts under `apps/weltraum-browser`. Playable flight v1 adds Flight State V2 with orientation, angular velocity, throttle, control mode, RCS/SAS state, command vectors and actuator telemetry. Manual controls and autopilot share `applyFlightControllerStep`, while `ChaseLocked` follows the moving ship and VFX/HUD read actuator telemetry. Cargo mass is a stubbed numeric field only; richer cargo/resource contracts remain deferred. |
| Non-goals | No Unity Rigidbody parity claim, no MonoBehaviour input lifecycle, no per-nozzle RCS allocator copy, no gimbal/SAS solver port, no cargo/resource/economy behavior, and no production GLB asset pipeline parity in playable-flight v1. |
| Tests / evidence hints | Unit tests cover deterministic fuel burn, mass-sensitive acceleration/braking reserve, no fuel, no autopilot authority, no main thrusters, brake-reserve insufficiency, HUD owner-snapshot consumption, plan-hash preservation, manual control mode/throttle/RCS/SAS behavior, shared actuator telemetry and no idle/cancel drift zero. Screenshot evidence covers manual ChaseLocked flight, RCS translation, autopilot thruster burn and autopilot arrival. |
...
| Browser-native intent | HUD renders read-only snapshots and player-facing ViewModels: control mode, camera mode, fuel/throttle/RCS/SAS, velocity/speed, selected target, route preview/locked-route state, compact radar-style route contact, warnings, target state, keybind help and next action. Debug/TestBridge surfaces stay labeled and separate from player UI. |
...
| Tests / evidence hints | Browser E2E checks that HUD reflects telemetry after fixed-step advances, warning chips show owner/reason, TestBridge is absent by default, target selection and route preview align with render snapshots, and screenshots cover selected target/autopilot active/warning/mobile states. |
