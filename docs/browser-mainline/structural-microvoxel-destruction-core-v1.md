...
ID and target object, expected/resulting object revision, the complete expected
Adaptive source/revision/epoch/digest binding, global or object-local quantized
shape, optional sorted material filter, actor, source, exactly one
...
Anchor/Joint facts. A component ID binds object ID, object revision, Structural content hash, the
complete Adaptive authority digest, the lexicographically smallest occupied-
cell key, and component content hash using Adaptive canonical hashing.
Detached components publish versioned Structural fragments with deterministic
IDs bound to component identity, revision, authority, and fragment content.
Anchor/Joint facts are indexed once under `maxIndexedFacts`; visited cells and
component count use separate named caps. Traversal or input-array order is not
identity. A-B-A means rehydrating the same persisted A revision; later equal
geometry at a newer revision may have different component IDs.
...
Mass properties come from occupied cells, never a mesh. Component mass first
rederives and exactly validates canonical identity, membership, content,
revision, object, and Adaptive authority binding. Products include total
...
Object address collection stops before reading cell
`maxVisitedCells + 1`. Foreign Components are descriptor-safely projected
before canonical comparison; membership is bounded by
`maxConnectivityCells`, active facts by `maxConnectivityFacts`, and the
mass address list by `maxVisitedCells`.
...
economy, multiplayer, dependency or runtime package changes,
Playwright/Vite configuration, or a second Adaptive authority. The sole
package-script change is Structural's exact-once test membership in the current
`test:e2e:core` group. Every future integration requires its own approved
contract and change.
