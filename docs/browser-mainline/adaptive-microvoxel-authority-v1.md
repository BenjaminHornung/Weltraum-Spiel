...
This is the first-publish V1 contract for the isolated Browser foundation. It
records the completed authority boundary and its normal-route proof; it does
not broaden that boundary into product integration.

## Scope and authority boundary

This document defines the implemented, renderer-independent authority
foundation for an adaptive microvoxel field in the Browser mainline. The
...
representation. Coordinate and key values are never represented as `BigInt`.
Internal exact arithmetic may use `BigInt` where the implementation requires
it, including hash arithmetic; that does not widen the accepted coordinate
domain. The maximum bound is excluded, so a coordinate on one brick's maximum
...
are rejected. Canonical object fields are sorted. Canonical numeric `-0` is
...
- package integration, package scripts, dependencies, or lockfiles;
...
document does not authorize changes to those boundaries.
