...
- Current navigation is local-space prototype guidance for waypoint routing, obstacle avoidance, trajectory preview, and HUD/debug visibility. Orbital navigation, patched conics, SOI planning, maneuver nodes, and gravity-assist/slingshot routing remain future work outside the current exact-arrival fix.
...
- The Autopilot Proving Ground harness is the current authority for autopilot quality. Its strict scenario evidence currently shows known exact-arrival bugs, so the waypoint autopilot should not be treated as fixed yet.
...
- The Autopilot Proving Ground harness is the quality source for waypoint autopilot claims. Current evidence still shows exact point-arrival failures, including 5-6.5 m direct-arrival error, terminal target loss, post-brake Accelerate/Reacquire flapping, disallowed no-obstacle Reacquire profiles, obstacle-corridor clearance violation, and excessive low-RCS terminal replanning.
