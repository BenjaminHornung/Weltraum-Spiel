...
namespace PrimeTween {
    ...
    internal sealed class PrimeTweenManager : MonoBehaviour {
        ...
        [SerializeField, HideInInspector] internal List<TweenAnimation> currentTweenAnimationsPrefabStage = new List<TweenAnimation>();
        ...
        internal void ResetCurrentTweenAnimations(PlayModeStateChange stateChangeForRegularAnimations = PlayModeStateChange.EnteredEditMode) {
        }

        static bool HasTargetInPrefabState(TweenAnimation a) {
            Assert.IsNotNull(a);
            return false;
        }
        ...
        internal static PlayModeStateChange? _playModeState;
        ...
        static void IniOnLoad() {
            ...
            AssemblyReloadEvents.afterAssemblyReload += () => {
                var newState = EditorApplication.isPlayingOrWillChangePlaymode ? PlayModeStateChange.EnteredPlayMode : PlayModeStateChange.EnteredEditMode;
                if (_playModeState != newState) {
                    DebugLifetimeStatic($"AfterAssemblyReload _playModeState = {newState}");
                    _playModeState = newState;
                }
            };
            ...
            EditorApplication.playModeStateChanged += state => {
                ...
                switch (state) {
                    ...
                    case PlayModeStateChange.ExitingEditMode:
                        ...
                        if (HasInstance) {
                            _instance.ResetCurrentTweenAnimations(state); // passing the current state allows resetting animations in the current state instead of the default PlayModeStateChange.EnteredEditMode
                            _instance.Dispose();
            ...
            if (Application.isPlaying) {
                ...
                DebugLifetimeStatic($"Application.isPlaying, set _playModeState: {_playModeState}");
            }
        ...
        internal static Tween? DelayWithoutDurationCheck([CanBeNull] object target, float duration, bool useUnscaledTime, bool isCallback = true) {
            ...
            tween.Setup(target, ref settings, false, isCallback ? TweenType.Callback : TweenType.Delay, ref rt, ref d);
        ...
        static bool IsInPrefabStage([CanBeNull] GameObject go) {
            #if UNITY_EDITOR
