...
public partial class Tests {
    ...
    public IEnumerator CoroutineEnumeratorMultipleToYieldInstruction() {
        ...
        LogAssert.Expect(LogType.Error, Constants.coroutineFinishedError);
        Assert.IsFalse(e.MoveNext());
        testCompletedCorEnumerator(e);
    }
    ...
    public IEnumerator ZeroDurationWithStartDelay() {
        ...
        PrimeTweenConfig.warnZeroDuration = false;
        float val = 0f;
        var t = Tween.Custom(new TweenSettings<float>(-1f, 1f, 0f, startDelay: 1f, endDelay: 1f), x => val = x);
        yield return null;
        Assert.IsTrue(t.isAlive);
        t.elapsedTime = 0.9f;
    ...
    public IEnumerator LateUpdateTweenDoesNotUpdateFirstTimeInTheSameFrame() {
        float val = 0f;
        Tween.Custom(new TweenSettings<float>(0f, 1f, minDuration, updateType: UpdateType.LateUpdate), v => {
    ...
    public void CallbackTweenProgress() {
        ...
        Assert.AreEqual(TweenAnimation.TweenType.Callback, callbackTween.tween.data.tweenType);
    ...
    public void CoroutineReuse() {
        ...
        Assert.AreNotEqual(it, t1.ToYieldInstruction()); // it's allowed to call ToYieldInstruction() multiple times on the same animation and the returned iterator should be different every time
    ...
    [Test]
    public void OnCompleteDoesntInvokeOnNegativeTimescale() {
        var t = Tween.Delay(1f).OnComplete(Assert.Fail);
        t.progressTotal = 0.5f;
        t.timeScale = -1;
        t.Complete();
    }
